...
updaterc() {
    ...
    if [ "${ADD_NVM_USE}" = "true" ]; then
        if [[ "$(cat /etc/bash.bashrc)" != *"$2"* ]]; then
            echo -e "$2" >> /etc/bash.bashrc
        fi
        if [ -f "/etc/zsh/zshrc" ] && [[ "$(cat /etc/zsh/zshrc)" != *"$3"* ]]; then
            echo -e "$3" >> /etc/zsh/zshrc
        fi
    fi 
}
...
if [ ! -d "${NVM_DIR}" ]; then
    ...
    # Update rc files
    if [ "${UPDATE_RC}" = "true" ]; then
        updaterc "${nvm_rc_snippet}" "${bashrc_nvm_use_snippet}" "${zshrc_nvm_use_snippet}"
    fi
else
